...
<aside class="min-w-64 max-w-72 bg-white border-r border-gray-200 h-screen overflow-y-auto p-4 pb-12 flex flex-col">
    ...
    <input
        ...
        bind:value={search}
    />
    <ul class="list-none p-0 w-full pb-8">
        {#each filteredCategories as cat}
            <li class="mb-0 border-b border-gray-100">
                ...
                >
                    ...
                    {/if}
                    <span class="flex-1 text-left ml-2 text-[13px]">
                        {@html highlight(cat.table, search)}
                    </span>
                </button>
                {#if cat.columns && (search ? autoExpanded[cat.table] : expanded[cat.table])}
                    <ul class="ml-4 mt-1 border-l border-gray-100 pl-2 pb-2">
